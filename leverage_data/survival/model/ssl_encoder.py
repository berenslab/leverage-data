...
def retfound_encoder(img_size=224, root = ''):
    ...
    # weights downloaded from https://drive.google.com/uc?id=1l62zbWUFTlp214SvK6eMwPQZAzcwoeBE on 22 May, 
    # 2025 from https://huggingface.co/open-eye/RETFound_MAE
...
def load_dino_nako(weights_path = None, config_file = None, nako_model = True):
    ...
    # print('checkpoint number of epochs is', checkpoints['epoch'])
...
def load_swin(weights_path):
    print('\n \n in SWIN \n \n')
    from functools import partial
    from .swin_mae import (swin_mae, SwinMAE)
    from .lightning_helpers import MAELightning, OptimCfg

    patch_size = 4
    img_size = 224

    model = SwinMAE(img_size=img_size, patch_size=patch_size, in_chans=3,
    ...
    lit = load_lightning_model(weights_path, MAELightning, model)
    return lit.swin    
...
def get_encoder(weights_path:str=None,  device:str="cuda:0",
                 img_size = 224, eval_augur = False, args = None):
    print('weights_path in get_encoder', weights_path)
    # model_epoch = None
    embed_dim = 512
    backbone_model = None
    if (weights_path is None) or ('scratch' in weights_path):
    ...
    elif "tinc" in weights_path.lower() or "vicreg" in weights_path.lower():
        print("tinc")
        backbone = get_vicreg_model(weights_path, device)
        backbone_model = "tinc_vicreg_resnet18"
    ...
    elif ('arc_mae_vit_base_patch16__' in weights_path):

        print(f"\n \n ***in nako mae with weights path {weights_path} *** \n \n")
        ...
        backbone, embed_dim = retfound_encoder(img_size, args.cloud)
        backbone_model = "retfound_mae_vit"

    elif 'dino_nako' in weights_path.lower():
        from pathlib import Path
        print('in dino_nako_module')
        ...
        config_file = 'ssl_default_config.yaml'
        cfg = OmegaConf.load(config_file)
        _, teacher_backbone, embed_dim = build_model_from_cfg(cfg)
        dino_model, embed_dim = load_dinov2_weights(teacher_backbone, weights_path)
        backbone  = DINOBackbone(dino_model, use_cls=True)
